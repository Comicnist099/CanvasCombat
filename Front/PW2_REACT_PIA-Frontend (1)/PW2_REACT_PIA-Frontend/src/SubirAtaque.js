...
export function SubirAtaque() {
  ...
  const attackCharacterHandler = async (e) => {
    ...
    swal({
      title: "AVISO",
      text: "La imagen y la miniatura del ataque no podrán ser eliminadas ni editadas en cuanto sean subidas a la página, asi mismo el ataque no podrá ser eliminado de ninguna forma, presiona OK para estar de acuerdo.",
      icon: "warning",
      buttons: ["Cancelar", "OK"],
    }).then((respuesta) => {
      if (respuesta) {
                ...
                if (respJson.error == "Bad Request") {
                  ...
                  if (friendsUser.team == "0") {
                    const response3 = await fetch(
                      `/teams/6375936aad8bffc948b4d770`
                    );
                    body3 = await response3.json();
                  } else {
                    const response3 = await fetch(
                      `/teams/63759398ad8bffc948b4d778`
                    );
                    body3 = await response3.json();
                  }
                  ...
                  const response = await fetch(`/teams/${body3._id}`, {
                    method: "PATCH",
                    headers: {
                      "Content-Type": "application/json",
                    },
                  ...
                  const respJson = await response.json();
                  swal({
                    text: `El ataque se ha actualizado con éxito. Sus puntos obtenidos son: ${puntuacion}`,
                    icon: "success",
                    buttons: "OK",
                  }).then((respuesta) => {
                    if (respuesta) {
